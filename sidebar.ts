export default {
  '/components': [
    {
      children: [
        {
          title: '组件总览',
          link: '/components',
        },
      ],
    },
    {
      title: '基础组件',
      children: [
        {
          title: 'LCaptchaButton-按钮倒计时',
          link: '/components/l-captcha-button',
        },
        {
          title: 'LNumberRoll-数值加载器',
          link: '/components/l-number-roll',
        },
        {
          title: 'LTrigger-弹出选择',
          link: '/components/l-trigger',
        },
        {
          title: 'LCardGroup-卡片选择',
          link: '/components/l-card-group',
        },
        {
          title: 'LTagGroup-标签选择',
          link: '/components/l-tag-group',
        },
        {
          title: 'LSliderVerify-前端滑块验证',
          link: '/components/l-slider-verify',
        },
        {
          title: 'LTianaiCaptcha-高级滑块验证',
          link: '/components/l-tianai-captcha',
        },
        {
          title: 'LRightMenu-右键菜单',
          link: '/components/l-right-menu',
        },
      ],
    },
    {
      title: '数据展示',
      children: [
        {
          title: 'LECharts-echarts容器组件',
          link: '/components/l-echarts',
        },
        {
          title: 'LCard-高级卡片',
          link: '/components/l-card',
        },
        {
          title: 'LTable-高级表格',
          link: '/components/l-table',
        },
        {
          title: 'LEditTable-高级编辑表格',
          link: '/components/l-edit-table',
        },
        {
          title: 'LDescriptions-高级描述列表',
          link: '/components/l-descriptions',
        },
        {
          title: 'LList-高级列表',
          link: '/components/l-list',
        },
        {
          title: 'LSeamlessScroll-无缝滚动',
          link: '/components/l-seamless-scroll',
        },
        {
          title: 'LCollapseCard-折叠卡片',
          link: '/components/l-collapse-card',
        },
        {
          title: 'LTreeTable-树表格',
          link: '/components/l-tree-table',
        },
        {
          title: 'LHighlightText-文本高亮',
          link: '/components/l-highlight-text',
        },
        {
          title: 'LSkeleton-高级骨架屏',
          link: '/components/l-skeleton',
        },
      ],
    },
    {
      title: '高级表单',
      children: [
        {
          title: 'LForm-高级表单',
          link: '/components/l-form',
        },
        {
          title: 'LModalForm-高级弹窗表单',
          link: '/components/l-modal-form',
        },
        {
          title: 'LDrawerForm-高级抽屉表单',
          link: '/components/l-drawer-form',
        },
        {
          title: 'LQueryForm-高级查询表单',
          link: '/components/l-query-form',
        },
        {
          title: 'LStepsForm-高级步骤表单',
          link: '/components/l-steps-form',
        },
        {
          title: 'LListForm-高级动态表单项',
          link: '/components/l-list-form',
        },
      ],
    },

    {
      title: '高级表单项',
      children: [
        {
          title: 'LFormItem-高级表单项',
          link: '/components/l-form-item',
        },
        {
          title: 'Input-输入框',
          link: '/components/l-form-item-input',
        },
        {
          title: 'Number-数字输入框',
          link: '/components/l-form-item-number',
        },
        {
          title: 'NumberRange-数字范围输入框',
          link: '/components/l-form-item-number-range',
        },
        {
          title: 'Password-密码输入框',
          link: '/components/l-form-item-password',
        },
        {
          title: 'TextArea-多行输入框',
          link: '/components/l-form-item-textarea',
        },
        {
          title: 'Captcha-验证码输入框',
          link: '/components/l-form-item-captcha',
        },
        {
          title: 'AutoComplete-联想输入框',
          link: '/components/l-form-item-auto-complete',
        },
        {
          title: 'Mentions-提及输入框',
          link: '/components/l-form-item-mentions',
        },
        {
          title: 'Select-下拉框',
          link: '/components/l-form-item-select',
        },
        {
          title: 'Checkbox-多选框',
          link: '/components/l-form-item-checkbox',
        },
        {
          title: 'Radio-单选框',
          link: '/components/l-form-item-radio',
        },
        {
          title: 'Cascader-级联选择',
          link: '/components/l-form-item-cascader',
        },
        {
          title: 'TreeSelect-树形选择',
          link: '/components/l-form-item-tree-select',
        },
        {
          title: 'Composition-组合选择',
          link: '/components/l-form-item-composition',
        },
        {
          title: 'Color-颜色选择',
          link: '/components/l-form-item-color',
        },
        {
          title: 'Switch-开关选择',
          link: '/components/l-form-item-switch',
        },
        {
          title: 'DatePicker-日期选择',
          link: '/components/l-form-item-date',
        },
        {
          title: 'TimePicker-时间选择',
          link: '/components/l-form-item-time',
        },
        {
          title: 'Slider-滑动条选择',
          link: '/components/l-form-item-slider',
        },
        {
          title: 'Segmented-分段器选择',
          link: '/components/l-form-item-segmented',
        },
        {
          title: 'Rate-评分选择',
          link: '/components/l-form-item-rate',
        },
        {
          title: 'Upload-文件上传',
          link: '/components/l-form-item-upload',
        },
        {
          title: 'Transfer-穿梭框',
          link: '/components/l-form-item-transfer',
        },
      ],
    },
  ],
  '/scene': [
    {
      children: [
        {
          title: '场景案例',
          link: '/scene',
        },
      ],
    },
    {
      children: [
        {
          title: '表格数据的导出',
          link: '/scene/table-export',
        },
        {
          title: '解析 Excel 数据成 JSON',
          link: '/scene/excel-import',
        },
        {
          title: 'Mock 模拟数据',
          link: '/scene/mockjs',
        },
        {
          title: '下载文件',
          link: '/scene/download',
        },
        {
          title: '大屏适配',
          link: '/scene/big-screen',
        },
      ],
    },
  ],
};

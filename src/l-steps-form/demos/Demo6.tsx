  ...
  return (
    <div>
      <Button type="primary" onClick={() => setOpen1(true)}>
        不合并每个表单的值并且LStepsForm.StepForm 没有设置 name 属性,以每个 LStepsForm.StepForm 的索引作为键
      </Button>
      <br />
      <br />
      <br />
      <Button type="primary" onClick={() => setOpen2(true)}>
        不合并每个表单的值并且LStepsForm.StepForm 设置 name 属性,以每个 LStepsForm.StepForm 的name作为键
      </Button>
      ...
      >
        ...
        <LStepsForm.StepForm name="step2-24" title="步骤2">
          <LFormItemInput name="phone" label="手机号" required tooltip="禁止空格 只能输入数字" type="phone" />
        </LStepsForm.StepForm>
      </LStepsForm>
      <LStepsForm
        isMergeValues={false}
      ...
      >
        <LStepsForm.StepForm title="步骤1" name="fsd">
          <LFormItemInput name="name11" label="名字1" required tooltip="禁止空格" />
          <LFormItemInput name="name21" label="名字2" required tooltip="禁止空格" />
        </LStepsForm.StepForm>
        <LStepsForm.StepForm title="步骤2" name="123jhg123">
          <LFormItemInput name="phone4" label="手机号" required tooltip="禁止空格 只能输入数字" type="phone" />
        </LStepsForm.StepForm>
      </LStepsForm>
    </div>
  );

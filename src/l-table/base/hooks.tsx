...
import { getTableColumnsKey, isEvenNumber, isFunction } from 'lighting-design/utils';
...
export function useFillSpace({
  ...
  useLayoutEffect(() => {
    ...
    const { bottom: antdTableBottom, height: antdTableHeight } = antdTableDom!.getBoundingClientRect();
 ...
 * 设置表格大小
 */
export function useTableSize(outSize: SizeType): [SizeType, React.Dispatch<React.SetStateAction<SizeType>>] {
...
export function useTableColumn({
    ...
    return innerColumns.map((col: Record<string, any>) => {
      const item: Record<string, any> = {
        ...
        onHeaderCell(c: Record<string, any>[]) {
          ...
          const innerHeaderCellStyle = typeof headerCellStyle === 'function' ? headerCellStyle(c) : headerCellStyle;
          return {
            ...headerCellProps,
            style: {
        ...
        onCell(record: Record<string, any>, rowIndex: number) {
          ...
          const innerCellStyle = typeof cellStyle === 'function' ? cellStyle(record, rowIndex) : cellStyle;
          return {
            ...cellProps,
            style: {
              ...cellProps?.style,
      ...
      if (item.toolTip && !item.ellipsis) {
        return {
          ...
          render: (text: any, row: Record<string, any>, i: number) => {
            ...
            return (
              <Tooltip placement="topLeft" title={val} {...(isPlainObject(item.toolTip) ? item.toolTip : {})}>
  ...
  const finalColumns = useMemo(() => {
    ...
    const newColumns = outColumns.filter((item: any, i) => columnKeys?.includes(getTableColumnsKey(item, i)));
    return newColumns;
  }, [
    columnKeys,
...
export function useMergePagination(outPagination?: Record<string, any> | false) {
  ...
  // 默认一页10条
  const outDefaultPageSize = useMemo(() => (outPagination && outPagination.defaultPageSize) || 10, []);
  return {
    outDefaultCurrent,
    outDefaultPageSize,
  };
 ...
 */
export function useMergeToolbarActionConfig(actionConfig: Record<string, any> | false): false | Record<string, any> {
...
export function useInitTable({
  ...
  useEffect(() => {
    ...
    if (requestCacheKey && requestCacheParams[0]) {
      const { current: initCurrent, pageSize: initPageSize, formValues: initFormValues } = requestCacheParams[0];
      current = initCurrent;
...
export function useOnHeaderRow({
  ...
  const headerRowFn = (...args: any[]) => {
    ...
    const innerHeaderRowStyle = typeof headerRowStyle === 'function' ? headerRowStyle?.(...args) : headerRowStyle;
